...
function TotemTimers.CreateTimers()
			  ...
			  (not AvailableSpells[lastSpell] and not AvailableSpells[NameToSpellID[lastSpell]]) then
                ...
                for k, v in pairs(TotemData) do
					if AvailableSpells[k] and v.element == self.nr then
						self.button:SetAttribute("*spell1", TotemTimers.GetMaxRank(k))
		...
		tt.Update = function(self, elapsed, ...)
			...
			if (self.timeElapsed > 0.2) then
				...
				if self.timers[1] > 0 and (self.totemRange or self.activeTotem == 8512 or  self.activeTotem == 8227)  then -- windfury $ Flametongue
					...
					if TotemTimers.ActiveProfile.CheckRaidRange then
						for i = 1, 4 do
 ...
 --[[              
        local TotemUpdate(self, elapsed)
            XiTimers.Update(self, elapsed)
			...
			if  not self.activeTotem or TotemData[self.activeTotem].noRangeCheck then
...
local TotemicCall = SpellIDs.TotemicCall
            ...
            for nr, spell in pairs(Cooldowns[self.timer.nr]) do
                ...
                if AvailableSpells[spell] then
                    ...
                    elseif duration > 2 then
                        ...
                        self.timer.timerBars[nr].icon:SetTexture(GetSpellTexture(spell))
                    end
...
UpdatePartyRange = function(timer, unit) --, unitGUID, enchantID)
	...
	elseif (timer.totemRange) then
		...
		local buffs = TotemTimers.UnitBuffs(unit)
        for j, buff in pairs(buffs) do	
            if buff.name == timer.totemBuff then
                inRange = true
                break
            end
        end
